...
class SemanticCheck:
    ...
    def checkSynPos(self, query, word, synonyms, index):
        ...
        for tag in tagged_sent:
            if tag[0] == word and index == count:
                ...
                count += 1
                continue
            if tag[0] in synonyms and tag[1] == req_pos:
                acceptable_syns.append(tag[0])
            count += 1
        return acceptable_syns
        
        
    def calcSemanticScore(self, query, word, synonyms, index):
        """Caclulates the semantic similarity for the sentences with syns substituted.
